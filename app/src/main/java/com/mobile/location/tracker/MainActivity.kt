	...
	Column(modifier = Modifier.fillMaxSize()) {
		Box(modifier = Modifier.weight(1f)) {
			...
			Column(
				...
				IntervalOption("10 sec", 10000L, selectedInterval) { selectedInterval = it }
				IntervalOption("60 sec", 60000L, selectedInterval) { selectedInterval = it }
				IntervalOption("5 min", 300000L, selectedInterval) { selectedInterval = it }
...
@Composable
fun OSMView(latestLocation: LocationEntity?) {
	AndroidView(
		factory = { context ->
